...
if(words == "!goldvip") then
local price = 1000000
if doPlayerRemoveItem(cid, 4863, 1) == TRUE then
local days = 15
...
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Foram adicionados ".. days .." dias de GOLD VIP no seu character.")
setPlayerStorageValue(cid, 13541, time)

local quantity = math.floor((getPlayerStorageValue(cid, 13541) - timenow)/(24 * 60 * 60))
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você tem ".. quantity .." dias de GOLD VIP restantes.")
else
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você precisa do item para comprar a GOLD VIP.")
end

elseif(words == "!golddays") then
local timenow = os.time()
local quantity = math.floor((getPlayerStorageValue(cid, 13541) - timenow)/(24 * 60 * 60))
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Você tem ".. (quantity < 0 and 0 or quantity) .." dias de GOLD VIP no seu character.")
